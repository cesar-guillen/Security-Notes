...
## Key Escrow
*Key escrow* is the process of placing a copy of a private key in a safe environment. This is useful for recovery. If the original key is lost, the organization retrieves the copy of the key to access the data. Some ways to accomplish this are:

* Leave the key to a trusted third party
* Have an employee keep the key
* Use a key recovery agent.

## Key Management
*Key management* within a PKI refers to all the steps taken to manage public and private keys used within the PKI. 

A *Key Management System (KMS)* is a centralized system or service responsible for the secure management of cryptographic keys used in various security applications. The KMS provides a secure framework for managing the entire lifecycle of keys. Some of the tasks handled by the KMS include.
...
* **Machine/Computer**: Issued to a device or computer. Commonly used to authenticate computers inside of a domain.
* **User**: Issued to a user. They can be issued for encryption, authentication, smart cards, and more.
* **Email**: Used for [[4. Using Cryptographic Protocols#Protecting Email|encrypting email communications]] and digital signatures.
* **Code Signing**: Developers can use code signing certificates to validate the integrity of executable applications or scripts.  ^1558ca
* **Self-Signed**: It is not issued by a trusted certificate. Private CAs within an enterprise often create self-signed certificates. Not trusted by default. Administrators often use [[5. Automation & Orchestration for Secure Operations|automation]] tools to copy the self-singed certificate into the root of the CA. 
* **Root**: The root certificate is the highest-level certificate issued by the root CA.
* **Wildcard**: Starts with an asterisk (\*\) and can be used for multiple sub domains as long as they have the same root domain name. 
* **Subject Alternative Name**: A *SAN* certificate is used for multiple domains that have different names but are owned by the same organization.
* **Domain Validation**: Indicates that the certificate requestor has some control over the DNS domain. The CA takes extra steps to contact the requestor to verify evidence.
* **Extended Validation**: *EV* certificates use additional steps beyond domain validation. Was used by browsers but not so much now.
...
![[Pasted image 20251110203418.png]]
