...
* **Internet Control Message Protocol (ICMP)**: Used to test basic connectivity and includes tools like ping and tracert. Due to the use of ICMP in [[1. Identifying Network Attacks#Denial of Service Attacks|DoS attack]] it is commonly blocked at [[3. Basic Network Infrastructure#Firewalls#Stateless Firewall Rules|firewalls]] and routers.  
* **Address Resolution Protocol (ARP)**: Resolves IPv4 addresses to MAC addresses. TCP/IP uses the IP address to get a packet to a destination network. It then uses the [[1. Basic Network Concepts#Addresses|MAC]] address to get it to the correct host. In other words, ARP is required once the packet reaches the destination subnet. ^bc4da6
